...
import { getErrorMessage, formatNaira, NG_STATES, SCHOOL_TYPES, BILLING_CYCLES } from "@/lib/utils";
...
export default function OnboardingPage() {
  ...
  const [form, setForm] = useState({
    ...
    subscriptionPlanId: "", duration: "1", owner: "", ownerEmail: "",
  ...
  // Must come after `form` is declared above — both reference form.subscriptionPlanId.
  const selectedPlan = activePlans.find((p) => String(p.id) === form.subscriptionPlanId);
  ...
  const submit = async () => {
    ...
    if (!form.duration || Number(form.duration) < 1) return toast.error("Enter how many cycles this term runs for");
    ...
    return (
      <div>
        ...
        <Card className="mx-auto max-w-md p-10 text-center">
          ...
          <Button className="mt-6" variant="outline" onClick={() => { setDone(null); setForm({ name:"",sub:"",rcNumber:"",schoolType:"",yearEstablished:"",motto:"",state:"",lga:"",phone:"",email:"",subscriptionPlanId:"",duration:"1",owner:"",ownerEmail:"",gradingSystem:"WAEC (A1–F9)" }); setSubTouched(false); }}>
        ...
        <CardBody>
          ...
          <div className="mt-2 flex items-end gap-2">
            <Input
              ...
              type="number" min="1" value={form.duration} onChange={u("duration")}
              placeholder="1" containerClassName="max-w-[220px]"
            />
            {selectedPlan && Number(form.duration) > 0 && (
              <p className="pb-2.5 text-xs text-ink/45">
                = {Number(form.duration)} {cycle.unit}{Number(form.duration) === 1 ? "" : "s"} ({cycle.months * Number(form.duration)} months)
              </p>
            )}
          </div>
          ...
          <div className="mt-6 rounded-xl border border-line bg-paper p-4">
            ...
            <p className="mt-1.5 text-xs leading-relaxed text-ink/55">The school admin receives an invite email to set their own password. From their dashboard they can upload their logo, set brand colors, configure classes, add students and staff, and customise their grading system.</p>
          </div>

          <div className="mt-6 flex justify-end gap-3">
            <Button variant="subtle" onClick={() => setForm({ name:"",sub:"",rcNumber:"",schoolType:"",yearEstablished:"",motto:"",state:"",lga:"",phone:"",email:"",subscriptionPlanId:"",duration:"1",owner:"",ownerEmail:"",gradingSystem:"WAEC (A1–F9)" })}>Clear form</Button>
