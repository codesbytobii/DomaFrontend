...
export function useSubscriptionPlans() {
  const { data, error, isLoading, mutate } = useSWR("/platform/subscription-plans", fetcher);
  return { items: data?.data || [], error, isLoading, mutate };
}
...
// payload: { subscription_plan_id, duration } — duration is a count of the
// plan's own billing cycles (e.g. an annual plan + duration=2 = 2 years).
// No more free trial; a school's term is always an explicit length.
...
export const createSubscriptionPlan = (payload) => api.post("/platform/subscription-plans", payload).then(unwrap);
export const updateSubscriptionPlan = (id, payload) => api.patch(`/platform/subscription-plans/${id}`, payload).then(unwrap);
export const deleteSubscriptionPlan = (id) => api.delete(`/platform/subscription-plans/${id}`).then(unwrap);
